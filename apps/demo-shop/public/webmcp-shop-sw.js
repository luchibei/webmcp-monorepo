...
(() => {
  ...
  // ../../packages/webmcp-sdk/dist/index.js
  ...
  function fail(code, message, details, metadata) {
  ...
  function isToolResponse(value) {
    ...
    if (maybeResponse.ok === false) {
      return typeof maybeResponse.error?.code === "string" && typeof maybeResponse.error?.message === "string";
    }
    return false;
  }

  // ../../packages/webmcp-sw-runtime/dist/index.js
  var WEBMCP_TOOL_CALL = "WEBMCP_TOOL_CALL";
  var WEBMCP_TOOL_RESULT = "WEBMCP_TOOL_RESULT";
  function isObject2(value) {
    return Boolean(value) && typeof value === "object";
  }
  function isToolCallMessage(value) {
    return isObject2(value) && value.type === WEBMCP_TOOL_CALL;
  }
  ...
  function getMessageTarget2(event) {
  ...
  function extractToolName(payload) {
    ...
    if (isObject2(payload.payload) && typeof payload.payload.tool === "string") {
      return payload.payload.tool;
    }
    return null;
  }
  function extractRawInput(payload) {
    if (payload.input !== void 0) {
      return payload.input;
    }
    if (isObject2(payload.payload) && "input" in payload.payload) {
      return payload.payload.input;
    }
    return payload.payload;
  }
  function getModelContextLike(scope) {
    if (!isObject2(scope)) {
      return null;
    }
    const candidates = [
      ...
      scope.modelContext,
      scope.self?.agent,
      scope.self?.modelContext,
      scope.navigator?.modelContext
    ];
  ...
  function createSwRouter(handlers, options = {}) {
    const scope = options.scope ?? globalThis;
    const modelContext = getModelContextLike(scope);
    const onMessage = async (event) => {
      const target = getMessageTarget2(event);
      if (!target) {
        return;
      }
  ...
  var latestCheckoutState = null;
  function roundCurrency(value) {
    return Math.round(value * 100) / 100;
  }
  var router = createSwRouter({
    __syncCheckoutState: {
...
//# sourceMappingURL=webmcp-shop-sw.js.map